...
function renderHomeList() {
    ...
    // Map through the keys of appData to build the home screen dynamically
    Object.keys(appData).forEach(key => {
        const subject = appData[key];
        
        const cardHtml = `
            <button data-subject="${key}" class="subject-btn w-full bg-white border border-slate-100 shadow-sm hover:shadow-md transition-shadow rounded-2xl p-5 flex items-center justify-between text-left group">
    ...
    document.querySelectorAll('.subject-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const key = e.currentTarget.getAttribute('data-subject');
            openSubject(key);
        });
    });
}
...
function openSubject(subjectKey) {
    currentSubject = appData[subjectKey];
    ...
    document.getElementById('subject-page').scrollTop = 0;
}
