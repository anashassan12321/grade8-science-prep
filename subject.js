...
document.addEventListener('DOMContentLoaded', () => {
    ...
    currentSubject.topics.forEach((topic, tIndex) => {
        ...
        const qaHtml = topic.qa.map((qa) => `
            <div class="qa-card bg-slate-900/60 border border-slate-700 rounded-lg overflow-hidden mb-2 shadow-sm">
                <button class="qa-toggle-btn w-full text-left p-3 font-semibold text-slate-100 flex justify-between items-center hover:bg-slate-700/50 transition-colors">
                ...
                <div class="answer-content">
                    <div class="answer-inner bg-slate-700/50 px-3 pb-3 text-slate-300 text-sm">
                        <span class="font-bold text-slate-100">A:</span> ${qa.a}
                    </div>
                </div>
            </div>
        `).join('');
        ...
        const mcqHtml = topic.mcqs.map((mcq, mIndex) => {
            const optionsHtml = mcq.options.map((opt, oIndex) => `
                <button data-correct="${oIndex === mcq.correct}" class="mcq-btn w-full text-left p-3 border border-slate-700 rounded-lg mb-2 text-sm font-medium text-slate-300 hover:bg-slate-700/40 transition-colors">
                    ${opt}
                </button>
            `).join('');
            ...
            return `
                <div class="mb-5 last:mb-0">
                    <p class="font-bold text-slate-100 text-sm mb-3">${mIndex + 1}. ${mcq.q}</p>
        ...
        const cardHtml = `
            <div class="bg-slate-800 rounded-2xl shadow-lg border border-slate-700 overflow-hidden slide-in-right" style="animation-delay: ${tIndex * 0.1}s">
                ...
                <div class="p-4 space-y-5">
                    <div>
                        <div class="inline-block px-2 py-1 ${currentSubject.themeBg} ${currentSubject.themeText} text-xs font-bold rounded mb-2 uppercase tracking-wide border ${currentSubject.themeBorder}">Definition</div>
                        <p class="text-slate-300 text-sm mb-3">${topic.definition}</p>
                        <ul class="text-sm text-slate-400 space-y-1">
                            ${factsHtml}
                        </ul>
                    </div>
                    <div class="bg-slate-900/60 rounded-xl p-4 border border-slate-700 relative">
                        <div class="absolute -top-3 left-4 bg-amber-500/20 text-amber-300 text-xs font-bold px-2 py-1 rounded-full border border-amber-700/60 shadow-sm flex items-center gap-1">
                            👶 Explain Like I'm 5
                        </div>
                        <p class="text-sm text-slate-300 mt-2 italic">"${topic.eli5}"</p>
                    </div>
                    <div>
